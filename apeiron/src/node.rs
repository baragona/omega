...
/// Wire colors: the "kind" of edge in the interaction net.
#[derive(Clone, Copy, PartialEq, Eq, Hash, Debug)]
pub enum WireColor {
    /// Data flow (arguments, return values).
    Blue,
    /// Control flow (sequencing, continuations).
    Red,
    /// Scope (binder-body links, scope delimiters).
    Green,
}
...
impl Port {
    pub fn new(target: Ptr, slot: u8, color: WireColor) -> Self {
        Port { target, slot, color }
    }
...
pub enum OpCode {
    ...
    Barrier { scope: u32 },
    /// Explicit substitution (de Bruijn shift): 0=Principal, 1=Inner
    Lens { shift: i32 },
    /// Unification variable (unsolved): 0=Principal
    Future { constraint_id: u32 },
